...
const ProfileScreen = ({ route, navigation }) => {
  const idEmployee = route.params.employee;
  ...
  const API_USER_DETAIL = 'https://mam-api-6e353e4d80e7.herokuapp.com/users/'; 

  useEffect(() => {
    // Memanggil API saat komponen dimuat
    fetch(API_USER_DETAIL + idEmployee)
      .then((response) => response.json())  // Mengubah response menjadi JSON
      .then((data) => {
        // Filter hanya data yang dibutuhkan
        const { name, email, gender, departement, image } = data.data;
        setFormData({ name, email, gender, departement, image });
      })
      .catch((error) => {
  ...
  const handleSave = async () => {
    ...
    const API_USER_UPDATE = 'https://mam-api-6e353e4d80e7.herokuapp.com/users/' + idEmployee;

    try {
      const response = await fetch(API_USER_UPDATE, {
        method: 'PUT',
        headers: {
    ...
    <ScrollView style={{ flex: 1, padding: 20, marginTop: 10 }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20 }}>
        <TouchableOpacity style={style.ButtonAdd}>
          <Text style={style.TextButtonAdd}>Edit</Text>
        </TouchableOpacity>
      ...
      <View>
        ...
        <TouchableOpacity style={style.ButtonSave} onPress={handleSave} >
          <Text style={{ textAlign: 'center', color: 'white', fontSize: 15, fontWeight: 500 }}>Update</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
